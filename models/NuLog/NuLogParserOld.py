...
class LogParser:
    def __init__(self, indir, outdir, filters, k_values, log_format):
        self.path = indir
        self.logName = None
        ...
        self.time_train, self.time_test, self.dict_size = None, None, None
    ...
    def parse(self, logName, batch_size=5, mask_percentage=1.0, pad_len=150, N=1, d_model=256,
              dropout=0.1, lr=0.001, betas=(0.9, 0.999), weight_decay=0.005, nr_epochs=5, num_samples=0, step_size=100):
        ...
        logdata_frame = self.load_data()
        if not os.path.exists(self.savePath):
            os.makedirs(self.savePath)

        ### Make the dictionary
        time_train = time()
        df_len = logdata_frame.shape[0]
        data_tokenized = []
        for i in trange(0, df_len):
            tokenized = self.tokenizer.tokenize('<CLS> ' + logdata_frame.iloc[i].Content)
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_dataloader):
                ...
                if i % self.step_size == 5:
                    print("Epoch Step: %d / %d" % (i, len(test_dataloader)))
        ...
        self.time_test = time() - time_test

    def get_dataloaders(self, data_tokenized):
        transform_to_tensor = transforms.Lambda(lambda lst: torch.tensor(lst))
        train_test_length = int(len(data_tokenized) * 0.5)
        print("2. Traing test length: {}".format(train_test_length))

        train_data = MaskedDataset(data_tokenized[:train_test_length], self.tokenizer, mask_percentage=self.mask_percentage, transforms=transform_to_tensor,
        ...
        test_data = MaskedDataset(data_tokenized[train_test_length:], self.tokenizer, mask_percentage=self.mask_percentage, transforms=transform_to_tensor,
                                  pad_len=self.pad_len)
    ...
    def load_data(self):
        ...
        # print("0. Train file: {}".format(train_file))
        # print("0. Test file: {}".format(test_file))
        with open(train_file, 'r') as fin1:
            for line1 in fin1.readlines():
                # print(line1)            # 2012-01-10 17:56:32,tokyo-dc-rm,mib2d 97404 lacp info not found for ifl 509
                # print(regex.search(line1.strip()))
                match = regex.search(line1.strip())
                message = [match.group(header) for header in headers]
                log_messages.append(message)
                linecount += 1
        with open(test_file, 'r') as fin2:
            for line2 in fin2.readlines():
                # print(line2)
                match = regex.search(line2.strip())
                message = [match.group(header) for header in headers]
                log_messages.append(message)
                linecount += 1

        # print("1. Header: {}".format(headers))
        print("2. Number of lines: {}".format(linecount))
        # print(log_messages)
        logdata_frame = pd.DataFrame(log_messages, columns=headers)
        logdata_frame.insert(0, 'LineId', None)
        logdata_frame['LineId'] = [i + 1 for i in range(linecount)]
        return logdata_frame
