...
class LogParser:
    ...
    def parse(self, logName, batch_size=5, mask_percentage=1.0, pad_len=150, N=1, d_model=256,
        ...
        self.k_values = [int(self.dict_size * 0.1), int(self.dict_size * 0.5), self.dict_size]
        ...
        for k_val in self.k_values:
            ...
            for i, (x, y, ind) in enumerate(results):
                for j in range(len(x)):
                    if self.num_there(self.tokenizer.index2word[y[j]]):             # if word contain number --> variable
                        data_words.append("**")
                    else:                                                           # else check
                        if y[j] in x[j][-k_val:]:                                   # if predicted word has higher confidence than k other words --> constant
                            data_words.append(self.tokenizer.index2word[y[j]])
                        else:                                                       # else variable
                            data_words.append("**")

                    # if any(map(str.isdigit, self.tokenizer.index2word[y[j]])):
                    #     data_words.append("**")
                    # else:
                    #     if y[j] in x[j][-k_val:]:
                    #         data_words.append(self.tokenizer.index2word[y[j]])
