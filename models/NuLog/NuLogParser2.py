...
class LogTokenizer:
    ...
    def tokenize(self, sentence):
        ...
        filtered = re.split(self.filters, sentence)                                 # List of string with added ' ' among each word

        new_filtered = []
        count = 0
        for idx in range(len(filtered)):
            if filtered[idx] is not None and filtered[idx] != '':
                new_filtered.append(filtered[idx])
                self.addWord(filtered[idx])                                         ## Make dictionary
                new_filtered[count] = self.word2index[filtered[idx]]
                count += 1
        return new_filtered
...
class MaskedDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, mask_percentage=0.2, transforms=None, pad=0, pad_len=64):
        self.data = data
        self.padded_data = pad_sequences(deepcopy(data), maxlen=pad_len, dtype="long", truncating="post", padding="post")
    ...
    def get_sample_weights(self):
        ...
        data_token_idx_df = pd.DataFrame(deepcopy(self.padded_data))
        ...
        for column in range(self.pad_len):
            ...
            data_token_idx_df.iloc[:, column] = data_token_idx_df.iloc[:, column].apply(lambda x: changeTokenToCount(x, storeColumnInfo[column]))
        weights = 1 - minmax_scale(data_token_idx_df.sum(axis=1), feature_range=(0.0, 0.75))
        #print("7. Weights: {}".format(weights))
        return weights
    ...
    def __getitem__(self, index):
        masked_data = deepcopy(self.padded_data[index])
        # print(masked_data)
...
class LogParser:
    def __init__(self,indir, outdir, filters, k, log_format):
        ...
        self.df_len, self.df_log = None, None
    ...
    def generate_logformat_regex(self, logformat):
        ...
        print("0. Regex: {}".format(regex))
        print("1. Header look like: {}".format(headers))
    ...
    def load_data(self):
        ...
        log_messages = []
        linecount = 0
        with open(os.path.join(self.path, self.logName), 'r') as fin:
            for line in fin.readlines():
                try:
                    ...
                    pass
            print("2. Number of log messages: {}".format(linecount))
        self.df_len = linecount
    ...
    def get_dataloaders(self, data_tokenized):
        ...
        weights = train_data.get_sample_weights()               # 1D- numpy array with 2000 weights [0.43, 0.122, 0.44,....]
        train_sampler = None
    ...
    def run_epoch(self, dataloader, model, loss_compute):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if i % self.step_size == 1:
                ...
                print("Epoch Step: %d / %d Loss: %f Tokens per Sec: %f" % (i, len(dataloader), loss / batch.ntokens, tokens / elapsed))
                start = time.time()
                tokens = 0
    ...
    def parse(self, logName, batch_size=20, mask_percentage=1.0, pad_len=256, N=1, d_model=256,
              dropout=0.1,  lr=0.001, betas=(0.9, 0.999), weight_decay=0.005, nr_epochs=10, num_samples=0, step_size=10):
