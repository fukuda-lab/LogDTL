...
class LogParser():
    def __init__(self, indir, outdir, log_format, minEventCount=2, merge_percent=1, rex=(), keep_para=True):
    ...
    def parse(self, logname):
        ...
        print('1. Parsing file: ' + os.path.join(self.path, logname))
    ...
    def tokenize(self):
        ...
        for idx, log in self.df_log['Content_'].iteritems():
            ...
            if idx == 0:
                print("6. Content: {}, Tokenize".format(log, self.bins))
        print("7. Done tokenize")

    def categorize(self):
        '''
            Abstract templates bin by bin
        '''
        print("6.5 Bin is: {}".format(self.bins))
        for key in self.bins:
            print("7. Key in bins: {}".format(key))
            abin = self.bins[key]
            abin['Events'] = []
        ...
        print("6.6 Done Category")

    def reconcile(self):
        '''
            Merge events if a bin has too many events
        '''
        print("6.7 Start Reconcile")
        for key in self.bins:
            abin = self.bins[key]
    ...
    def load_data(self):
        print("2. Start loading datafile.")
        def preprocess(log):
        ...
        print("5. Content of the log: {}".format(self.df_log['Content_']))
    ...
    def generate_logformat_regex(self, logformat):
        ...
        print("3. Header and Regex: {}, {}".format(headers, regex))
